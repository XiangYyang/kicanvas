        ...
        if (layer.name === LayerNames.net_name) {
            ...
            if (pad_min_size > 0.5) {
                ...
                if (pad.net) {
                    ...
                    // TODO: calcuate the font size
                    font_attr.size = new Vec2(font_size, font_size);
                    font_attr.stroke_width =
                        font_size < 1000 ? 100 : font_size / 8;
                    StrokeFont.default().draw(
                        this.gfx,
                        net_name,
                        new Vec2(0, font_size / 2),
                        font_attr,
                    );
                }
            }
            ...
            if (is_hole_layer && pad.drill != null) {
                ...
                switch (shape) {
                    case "circle":
                        this.gfx.circle(
                            new Circle(center, pad.size.x / 2, color),
                        );
                        break;
                    case "rect":
                        {
                    ...
                    case "trapezoid":
                        ...
                        // and a polyline.
                        {
                            ...
                            half_size = half_size.sub(
                                new Vec2(rounding, rounding),
                            );
...
class GrTextPainter extends BoardItemPainter {
    ...
    paint(layer: ViewLayer, t: board_items.GrText) {
        ...
        console.log(edatext.attributes);
